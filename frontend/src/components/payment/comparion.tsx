...
const FeatureComparison = () => {
  return (
    <section id="featureComparison" className="py-20 bg-neutral-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16 animate__animated animate__fadeIn">
          <h2 className="text-3xl font-bold text-gray-900">Compare Features Across Plans</h2>
          <p className="mt-4 text-xl text-gray-600">Find the perfect plan for your business needs</p>
        </div>

        <div className="overflow-x-auto">
          <table className="w-full border-collapse bg-white shadow-lg rounded-lg overflow-hidden animate__animated animate__fadeInUp">
            <thead>
              <tr className="bg-neutral-900 text-white">
                ...
                {plans.map((plan, index) => (
                  <th
                    key={index}
                    className={`px-6 py-4 text-center ${plan.highlight ? "bg-blue-600" : ""}`}
                  >
                    {plan.name}
                  </th>
                ))}
              ...
              {features.map((feature, index) => (
                <tr key={index} className="border-b border-gray-200 hover:bg-gray-50">
                  ...
                  {feature.values.map((value, i) => (
                    <td key={i} className={`px-6 py-4 text-center ${plans[i].highlight ? "bg-blue-50" : ""}`}>
        ...
        <div className="mt-12 text-center">
          <button className="bg-blue-600 text-white px-8 py-3 rounded-lg font-semibold hover:bg-blue-700 transition-colors animate__animated animate__pulse animate__infinite">
